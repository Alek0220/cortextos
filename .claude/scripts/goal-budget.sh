...
#
# State lives at .claude/.goal-state.json — survives session compaction so
# the model can't lose track of how many iterations it's actually run.
...
if not os.path.exists(".claude/.goal-state.json"):
    sys.exit(0)  # no goal = nothing to budget = "exhausted"
s = json.load(open(".claude/.goal-state.json"))
